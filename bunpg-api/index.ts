...
async function main() {
    ...
    const pg_conn_str = `postgres://${process.env.A_PGUSER}:${accessToken.token}@${process.env.A_PGNAME}.postgres.database.azure.com/${process.env.A_PGDATABASE || "postgres"}?sslmode=require`
    console.log ("connecting to ", pg_conn_str)
    sql = new SQL(pg_conn_str);
